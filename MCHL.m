...
%                                                                         %
% This algorithm implements a novel multi-view clustering approach that   %
% combines topological structure information with manifold learning for   %
% improved clustering performance across multiple data views.             %
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [y, a, S, Tim, Obj] = MCHL(data,labels, alpha, beta, gama, knn, normData)
% data: cell array, view_num by 1, each array is num_samp by d_v
% num_clus: number of clusters
% num_view: number of views
% num_samp: number of sample
% k: number of adaptive neighbours
% labels: groundtruth of the data, num by 1
%
% y: predict label
% a: availability 1 is available， 0 is unavailable
% figure()
% [S1,~] = mapminmax(S, 0, 1)
% imshow(S1)
% figure()
...
I = eye(num_samp);
%initialize ed
%ed（i,j） is the distance between node i and node j
idxx = cell(1,num_view);
ed = cell(1,num_view);
for v = 1:num_view
    ed{v} = L2_distance_1(data{v}', data{v}');% cal distance between each node in data{v}
...
for iter = 1:NITER
    ...
    for v = 1:num_view
        ...
        Z{v} = zeros(num_samp,num_samp);
        for ni = 1:num_samp
            index = find(G{v}(ni,:)>0);  %before:index = find(c{v}(ni,:)>0)
            Ii = I(ni,index);
            si = S(ni,index);
            Ei = ed{v}(i,index);
            b = 2*alpha*Ii + 2*beta*mu(v)*si - gama*Ei;
            % solve z^T*A*z-z^T*b
